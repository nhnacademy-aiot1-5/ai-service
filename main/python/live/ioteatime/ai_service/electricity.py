...
def query_all(window_period, type, phase, description, fn):
...
def get_channel_hourly_usage(df):
...
# 전력 사용량
def get_total_hourly_usage(type):
    df = get('1h', type, 'kwh', 'sum', 'last')
    df_usage = pd.DataFrame(columns=['ds', 'y'])
...
# 시간별 w 평균
def get_hourly_electricity(type):
    return get('1h', type, 'total', 'w', 'mean')